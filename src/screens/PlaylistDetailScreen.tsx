...
import { doc, updateDoc, deleteDoc, setDoc,getDoc, increment, 
...
export default function PlaylistDetailScreen() {
  ...
  const [selectedTrackForOptions, setSelectedTrackForOptions] = useState<any>(null);
  ...
  const fetchTracks = async () => {
    try {
      ...
      if (auth.currentUser) {
        ...
        if (token) {
          try {
            const spotifyData = await getPlaylistTracks(token, currentPlaylist.id);
      ...
      try {
        const tracksRef = collection(db, "playlists", currentPlaylist.id, "tracks");
  ...
  const handleAddTrackToTargetPlaylist = async (targetPlaylist: any) => {
    ...
    try {
      ...
      const trackRef = doc(db, "playlists", targetPlaylist.id, "tracks", selectedTrackForOptions.id);
      const trackSnap = await getDoc(trackRef);

      if (trackSnap.exists()) {
        // Nếu có rồi thì báo và dừng luôn, không làm gì nữa
        Alert.alert("Đã tồn tại", `Bài hát này đã có trong playlist ${targetPlaylist.name}`);
...
// Handle Liked Function
  const handleLikeTrack = async (track: any) => {
    ...
    try {
      ...
      const likedRef = doc(db, "users", auth.currentUser.uid, "liked_songs", track.id);
    ...
    () => (
      <View style={styles.headerContainer}>
        ...
        >
          ...
          <TouchableOpacity style={styles.circleButton} activeOpacity={0.7} onPress={() => {
        ...
        >
          <View style={{ flex: 1 }}>
            ...
            >
              ...
              >
                <View style={[styles.modalView, { width: '100%', borderBottomLeftRadius: 0, borderBottomRightRadius: 0, paddingBottom: 40, marginTop: 'auto' }]}>
                  
                  {/* Header của Modal (Thông tin bài hát) */}
                  {selectedTrackForOptions && (
                      <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 20, borderBottomWidth: 0.5, borderBottomColor: '#444', paddingBottom: 15, width: '100%' }}>
                          <Image 
                            source={{ uri: selectedTrackForOptions.album.images[0]?.url }} 
                            style={{ width: 50, height: 50, borderRadius: 8, marginRight: 15 }} 
                          />
                          <View style={{ flex: 1 }}>
                              <Text style={{ color: 'white', fontWeight: 'bold', fontSize: 16 }}>{selectedTrackForOptions.name}</Text>
                              <Text style={{ color: '#ccc', fontSize: 14 }}>{selectedTrackForOptions.artists[0].name}</Text>
                          </View>
                      </View>
                  )}
                  ...
                  >
                     <Ionicons name="add-circle-outline" size={24} color="white" style={{ marginRight: 15 }} />
                  ...
                  >
                     <Ionicons name="heart-outline" size={24} color="white" style={{ marginRight: 15 }} />
                    ...
                    onPress={() => {
                        ...
                        Alert.alert(
                            ...
                            [
                                ...
                                { text: "Xóa", style: "destructive", onPress: () => handleDelete(selectedTrackForOptions) }
                            ]
                        );
                    }}
                  >
                     <Ionicons name="trash-outline" size={24} color="#FF1744" style={{ marginRight: 15 }} />
                     <Text style={[styles.name, { color: '#FF1744' }]}>Xóa khỏi Playlist</Text>
                  </TouchableOpacity>
              ...
              <View style={[styles.modalContainer, { paddingTop: 20 }]}>
                ...
                <View style={[styles.modalHeader, { borderBottomWidth: 0 }]}>
                  ...
                  <TouchableOpacity onPress={() => setAddToPlaylistModalVisible(false)}>
                    <Text style={{ color: "#E91E63", fontSize: 16, fontWeight: '600' }}>Hủy</Text>
                  </TouchableOpacity>
                </View>
